...
export default {
  ...
  mounted() {
    ...
    this.fetchSalesStoresCategoryDate({ by_store_id: this.$route.params.id })
      .then(response => {
        const dates = []
        const categories = []
        const datasets = []
        response.forEach(x => {
          if (!categories.includes(x.name)) {
        ...
        categories.forEach(category => {
          ...
          dates.forEach(date => {
            let total = 0
            response.forEach(x => {
              if (x.date === date && x.name === category) {
                total = x.total
              }
            })
            data.push(total)
          ...
          datasets.push({
            label: category,
            data,
            backgroundColor: graphColors[categories.indexOf(category)],
          })
        })
        this.graphData.data.labels = dates
        this.graphData.data.datasets = datasets
